...
git_init(){
	...
	if [ -f pointer ]; then
		# 如果文件存在,那么判断文件是否有值
		if [ -z `cat < pointer` ]; then
			echo "google_containers" > pointer
		fi
	else
		# 若是文件不存在,那么给这个文件一个初始值
		echo "google_containers" > pointer
	fi
}
...
image_pull(){
	...
	while read LINE; do
		...
		for I in $(seq $THREAD); do
			read -u5
			{
				#if [ $(echo $LINE | awk -F: '{print $1}') != "gcr.io/kubeflow-images-public/kaggle-notebook" -o $(echo $LINE | awk -F: '{print $1}') != "gcr.io/kubeflow-images-public/t2t-yelp-gpu" ]; then
...
main(){
	...
	for I in $GCRIO_NS $QUAYIO_NS; do
		...
		if [ $I == $POINTER ]; then
			...
			if [ `grep -A 1 "$POINTER" allns | wc -l` -eq 1 ]; then
				echo 'google_containers' > pointer
				cat pointer
				if [[ $GCRIO_NS =~ $I ]]; then
					image_list_create $I gcr.io
				...
				grep -A 1 "$POINTER" allns | tail -n1 > pointer
				cat pointer
				if [[ $GCRIO_NS =~ $I ]]; then
					image_list_create $I gcr.io
				else
